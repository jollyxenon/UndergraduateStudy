...
function [WaveFn, r] = Hydrogen_3d
%--------------------------------------------------------------------------
% define constants and orbital shape

% quantum numbers
n = 3;           % principal quantum number
l = 2;           % angular momentum quantum number (subshell type: s, p, d, f, etc.)
...
cons.spatialLen = 10;             % length of physical space to view over (angstroms)
...
clipRange = [nan,nan,nan,nan,nan,halfLength];
[x,y,z,v] = subvolume(xSpace,ySpace,zSpace,WaveFn,clipRange);

% maximum value
absv = abs(v);
vmax = max(max(max(absv)));
%v = -v;
...
isovalue = 0.1 * vmax;
p1 = patch(isosurface(x,y,z,v,isovalue),'FaceColor',cmap(12,:),'EdgeColor','none');
isonormals(x,y,z,v,p1);
patch(isocaps(x,y,z,v,isovalue,'above'),'FaceColor','interp','EdgeColor','none');

p2 = patch(isosurface(x,y,z,v,-isovalue),'FaceColor',cmap(10,:),'EdgeColor','none');
