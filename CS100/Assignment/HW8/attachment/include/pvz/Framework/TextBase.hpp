...
class TextBase {
 public:
  TextBase(int x, int y, const std::string& text = "", double colorR = 0,
           double colorG = 0, double colorB = 0, bool m_centering = true);
 ...
 private:
  ...
  static void DisplayAllTexts(Func displayFunc) {
    for (auto& obj : TextBase::s_allTexts) {
      displayFunc(obj->m_x, obj->m_y, obj->m_text, obj->m_colorR, obj->m_colorG,
                  obj->m_colorB, obj->m_centering);
    }
  }
